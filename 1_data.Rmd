...
## Juntamos atributos Building y Floor en una sola columna para posteriores comprobaciones en TRAINING y VALIDATION.
```{r}
...
```
En el DF training existen 472 capturas que tienen valores entre 0 y -30, lo cual es irreal.
56 WAPs tienen algun valor en ese rango.
AL BUSCAR CON UN LOOP EN EL VALIDATION, ENCONTRAMOS QUE NO HAY NINGUNA CAPTURA NI NINGUN WAP QUE TENGA VALOR ENTRE 0 Y -30. Esto nos hace pensar que algun problema ha habido al hacer el training, seguramente por los dispositivos. Estudiaremos su comportamiento y seguramente quitemos esos datos para entrenar el modelo.

## Estudiamos las capturas que tienen registros de valores entre -30 y 0 RSSI.
```{r}
...
df.cptr30 %>%
  ...
  summary()
```
El usuario que tiene mas errores de medicion es el USERID 6, seguido del 14.
Los lugares donde ocurren estos errores de medicion en el DF training son la planta 3 y 4 del edificio 2 mayoritariamente. La planta 4 es la mas problematica.
El USERID 14 utiliza un Samsung mini GT-S6500 con Android 2.3.6 Gingerbread, una version de Android ya muy antigua para un dispositivo moderno como era el año del estudio, lo cual podria estar produciendo errores.
Aun asi, el USERID 6 utiliza un Google Nexus 4 con Android 4.2.2 Kitkat que era la ultima version de Android en ese momento y este movil tiene muchos problemas de registros.
Podria ser que los moviles tuvieran problemas con el chip de Wifi. Tendremos que estudiar si hay mas observaciones en el training que pertenezcan al edificio 2 planta 3 y 4. Si las hay, ningun problema, si no hay, entonces la zona puede ser el problema (baja cobertura) si en el Validation hay registros.

## Comprobamos cuantas capturas tenemos por cada planta antes y despues de quitar las 472 capturas con problemas de registro de RSSI.
```{r}
...
```
Recordamos que solo el edificio 2 tiene 4 plantas. Aunque tiene muchos menos registros que las demas plantas, puede que sean suficientes para entrenar un modelo. Esto podria afectar al kappa.

## Identificamos y quitamos las filas que estan duplicadas.
```{r}
...
## Guardamos DFs para usarlos en el siguiente script
```{r}
