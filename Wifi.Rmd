...
```{r}
df.cptr30 = df.cptr30[,c(315:316, 319:320)]
table(df.cptr30$FLOOR)
table(df.cptr30$BUILDINGID)
table(df.cptr30$USERID) #El usuario que tiene mas errores de medicion es el USERID 6, seguido del 14.
...
## Comprobamos cuantas capturas tenemos por cada planta quitando previamente las 472 capturas con problemas de registro de RSSI.
```{r}
vec.cptr30 = as.integer(row.names(df.cptr30))
table(df.train[-vec.cptr30,]$FLOOR)
