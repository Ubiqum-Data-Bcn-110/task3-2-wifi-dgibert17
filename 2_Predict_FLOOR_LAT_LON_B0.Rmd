...
### Floor prediction error analysis
```{r}
...
for (i in 1:nrow(real.pred.values)) {
  if (real.pred.values[i,"long_diff"] <= 4 & real.pred.values[i,"lat_diff"] <= 4) {
    real.pred.values[i,"PREDICTION"] = "Awesome! :D"
  } else if (real.pred.values[i,"long_diff"] <= 12 & real.pred.values[i,"lat_diff"] <= 12) {
