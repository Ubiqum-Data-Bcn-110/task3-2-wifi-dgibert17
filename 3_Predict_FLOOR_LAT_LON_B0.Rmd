---
title: "Predicting FLOOR on building 0"
...
####################################################################################
#### PREDICCION DE LATITUDE - RF SIN DUMIFICAR A VER SI PREDICE MEJOR
```{r}
# bestmtry = tuneRF(x = df.train.B0.dumm[,c(1:139, 149:152)], y = df.train.B0.dumm$LATITUDE, ntreeTry = 300, plot = T,stepFactor = 1.5, improve = 1e-5)

# rf.fit.long = randomForest(x = df.train.B0.dumm[,c(1:139, 140, 149:152)], y = df.train.B0.dumm$LONGITUDE, ntree = 200, mtry = 22)
rf.fit.lat = randomForest(LATITUDE ~., df.train.B0[,c(1:139, 141, 142)], ntree = 100, mtry = 22)

rf.pred.lat = predict(rf.fit.lat, df.val.B0[,c(1:139, 142)])

rmse(actual = df.val.B0$LATITUDE, predicted = rf.pred.lat)
mae(actual = df.val.B0$LATITUDE, predicted = rf.pred.lat)
```
#####################################################################################
...
######################################################################
#### PREDICCION DE LONGITUDE - RF SIN DUMIFICAR, A VER SI PREDICE MEJOR QUE DUMIFICANDO FLOOR
```{r}
# bestmtry = tuneRF(x = df.train.B0[,c(1:139, 141, 142)], y = df.train.B0$LONGITUDE, ntreeTry = 200, plot = T,stepFactor = 1.5, improve = 1e-5)

rf.fit.long = randomForest(LONGITUDE ~., df.train.B0[,c(1:139, 140, 141, 142)], ntree = 200, mtry = 108)

rf.pred.long = predict(rf.fit.long, df.val.B0[,c(1:139, 141, 142)])

rmse(actual = df.val.B0$LONGITUDE, predicted = rf.pred.long)
mae(actual = df.val.B0$LONGITUDE, predicted = rf.pred.long)
```
######################################################################
